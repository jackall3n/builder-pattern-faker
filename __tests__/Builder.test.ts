...
describe('Builder', () => {
  ...
  describe('with faker', () => {
    it('should provider faker to template', () => {
      const builder = Builder<Testing>((faker: Faker) => ({
    ...
    it('should provider faker to props', () => {
      const template: Testing = {
        a: 10,
        b: 'abc',
        c: true
      ...
      const builder = Builder(template)
        .a(faker => faker.datatype.number())
        .b(faker => faker.datatype.string())
        .c(faker => faker.datatype.boolean());

      expect(builder.build()).toEqual({
        a: expect.any(Number),
        b: expect.any(String),
        c: expect.any(Boolean),
      });
    });
  });
});
